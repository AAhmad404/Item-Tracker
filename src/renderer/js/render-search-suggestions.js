    ...
    window.electron.on('search-suggestions', (data) => {
        ...
        if (data.error) {
            ...
            data.data.forEach(item => {
                const highlightedName = item.name.replace(new RegExp(`(${searchText})`, 'gi'), '<strong>$1</strong>');
                const highlightedLocation = item.location.replace(new RegExp(`(${searchText})`, 'gi'), '<strong>$1</strong>');
