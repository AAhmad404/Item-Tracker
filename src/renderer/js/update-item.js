$(document).ready(() => {
    ...
    $('#save-button').on('click', () => {
        ...
        const imageData = $('#item-information-image').css('background-image').replace(/url\(['"]?(.*?)['"]?\)/, '$1');

        // Check if the imageData is a data URL (base64 format)
        const isDataUrl = imageData.startsWith('data:image/png') || imageData.startsWith('data:image/jpeg');
    ...
    function validateForm() {
        ...
        const name = itemInformationNameInput.val().trim() || itemInformationNameInput.attr('placeholder');
        const location = itemInformationNameInput.val().trim() || itemInformationLocationInput.attr('placeholder');
        const count = itemInformationCountInput.val().trim() || itemInformationCountInput.attr('placeholder');

        if (!itemInformationNameInput || !itemInformationNameInput || !itemInformationCountInput) {
